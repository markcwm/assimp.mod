...
Rem
bbdoc: Assimp 4x4 matrix extended
EndRem
Type TaiMatrix4x4Ex Extends TaiMatrix4x4
	...
	Function Create:TaiMatrix4x4( pMat:Float Ptr )
	
		Local m:TaiMatrix4x4 = New TaiMatrix4x4
	...
	Method _Decompose()
		...
		Local rm:TaiMatrix3x3 = New TaiMatrix3x3
		...
		If (b1 > 0.998)
			...
			attitude = 90 'Pi/2
			bank = 0
			If TGlobal3D.Log_Assimp Then DebugLog " TaiMatrix4x4 Decompose: singularity at north pole"
			Return
		EndIf
		...
		If (b1 < -0.998)
			...
			attitude = -90 '-Pi/2
			bank = 0
			If TGlobal3D.Log_Assimp Then DebugLog " TaiMatrix4x4 Decompose: singularity at south pole"
			Return
		EndIf
...
Type TaiCamera ' Helper structure to describe a virtual camera
    Field mName:TaiString ' The name of the camera, there must be a node in the scenegraph with the same name
    Field mPosition:TaiVector3 ' Position of the camera relative to the coordinate space
    Field mUp:TaiVector3 ' 'Up' - vector of the camera coordinate system relative to the coordinate space
    Field mLookAt:TaiVector3 ' 'LookAt' - vector of the camera coordinate system relative to the coordinate space
...
Type TaiLight ' Helper structure to describe a light source
	Field mName:TaiString ' The name of the light source
    Field mType:Int ' The type of the light source
	Field mPosition:TaiVector3 ' Position of the light source in space
	Field mDirection:TaiVector3 ' Direction of the light source in space
	...
	Field mColorDiffuse:TaiColor3D ' Diffuse color of the light source
	Field mColorSpecular:TaiColor3D ' Specular color of the light source
	Field mColorAmbient:TaiColor3D ' Ambient color of the light source
...
Type TaiMetadataEntry ' Metadata entry, the type field uniquely identifies the underlying type of the data field
	Field mType:Int
	Field mData:Byte Ptr
End Type

Type TaiMetadata ' Metadata is a key-value store using string keys and values
	Field mNumProperties:Int ' Length of the mKeys and mValues arrays, respectively
	Field mKeys:TaiString[] ' Arrays of keys, may Not be Null, entries in this array may Not be Null
	Field mValues:TaiMetadataEntry[] ' Arrays of values, may Not be Null, array entries may be Null if key has no value
End Type

' texture.h

Type TaiTexel ' Helper structure to represent a texel in a ARGB8888 format, used by TaiTexture
	Field b:Byte, g:Byte, r:Byte, a:Byte
End Type

Type TaiTexture ' Helper structure to describe an embedded texture
	...
	Field pcData:TaiTexel[] ' Data of the texture, points to an array of mWidth * mHeight TaiTexel's
End Type

' types.h

Type TaiPlane ' Represents a plane in a three-dimensional, euclidean space
	Field a:Float, b:Float, c:Float, d:Float
End Type

Type TaiRay ' Represents a ray
	Field pos:TaiVector3, dir:TaiVector3 ' Position and direction of the ray
End Type

Type TaiColor3D ' Represents a color in Red-Green-Blue space
...
bbdoc: Assimp string
EndRem
Type TaiString ' Represents an UTF-8 string, zero byte terminated, used instead of std::string to be C-compatible
	...
	?
	Field data:Byte[MAXLEN] 'String buffer, size limit is MAXLEN (note TaiString size is 16 bytes so data is a pointer)
...
Type TaiMemoryInfo ' Stores the memory requirements for different components of an import, all sizes are in bytes
...
Type TaiUVTransform ' Defines how an UV channel is transformed, helper structure For the #AI_MATKEY_UVTRANSFORM key
    Field mTranslation:TaiVector2 ' Translation on the u and v axes, default value is (0|0)
    Field mScaling:TaiVector2 ' Scaling on the u and v axes, default value is (1|1)
    Field mRotation:Float ' Rotation in counter-clockwise direction, rotation angle is specified in radians
End Type

Type TaiMaterialProperty ' Data structure for a single material property, just use the TaiMaterial::Get() functions
	Field mKey:TaiString ' Specifies the name of the property (key)
...
Rem
bbdoc: Assimp material property extended
EndRem
Type TaiMaterialPropertyEx Extends TaiMaterialProperty
	...
	Function Create:TaiMaterialPropertyEx( pProps:Byte Ptr )
	
		Local mp:TaiMaterialPropertyEx = New TaiMaterialPropertyEx
		mp.mKey = New TaiString
...
Type TaiMaterial ' Material data is stored using a key-value structure, a single key-value pair is called a material property
	Field mProperties:TaiMaterialPropertyEx[] ' List of all material properties loaded
	Field mNumProperties:Int ' Number of properties in the data base
	Field mNumAllocated:Int ' Storage allocated
End Type

Rem
bbdoc: Assimp material extended
EndRem
Type TaiMaterialEx Extends TaiMaterial
	...
	Method GetMaterialString:String( Key:String )
		...
		Local as:TaiString = New TaiString
...
Type TaiVectorKey ' A time-value pair specifying a certain 3D vector for the given time
	Field mTime:Double ' The time of this key
	Field mValue:TaiVector3 ' The value of this key
End Type

Type TaiQuatKey ' A time-value pair specifying a quaternion rotation for the given time
	Field mTime:Double ' The time of this key
	Field mValue:TaiQuaternion ' The value of this key
End Type

Type TaiMeshKey ' Binds a anim mesh to a specific point in time
	Field mTime:Double ' The time of this key
	Field mValue:Int ' Index into the TaiMesh::mAnimMeshes array of the mesh corresponding to the #aiMeshAnim
End Type

Type TaiNodeAnim ' Describes the animation of a single node, the name specifies the bone/node which is affected
	Field mNodeName:TaiString ' The name of the node affected by this animation
	Field mNumPositionKeys:Int ' The number of position keys
	Field mPositionKeys:TaiVectorKey[] ' The position keys of this animation channel, specified as 3D vector
	Field mNumRotationKeys:Int ' The number of rotation keys
	Field mRotationKeys:TaiQuatKey[] ' The rotation keys of this animation channel, given as quaternions
	Field mNumScalingKeys:Int ' The number of scaling keys
	Field mScalingKeys:TaiVectorKey[] ' The scaling keys of this animation channel, specified as 3D vector
...
Type TaiMeshAnim ' Describes vertex-based animations for a single mesh or a group of meshes
    Field mName:TaiString ' Name of the mesh to be animated, an empty string is not allowed
    Field mNumKeys:Int ' Size of the #mKeys array. Must be 1, at least
    Field mKeys:TaiMeshKey[] ' Key frames of the animation, may not be Null
End Type

Type TaiAnimation ' An animation consists of keyframe data for a number of nodes
	Field mName:TaiString ' The name of the animation, should be empty if only a single animation channel
	...
	Field mChannels:TaiNodeAnim[] ' The node animation channels, each channel affects a single node
    Field mNumMeshChannels:Int ' The number of mesh animation channels, each channel affects a single mesh
    Field mMeshChannels:TaiMeshAnim[] ' The mesh animation channels, each channel affects a single mesh
End Type

' mesh.h

Type TaiFace ' A single face in a mesh, referring to multiple vertices
...
Type TaiVertexWeight ' A single influence of a bone on a vertex
...
Type TaiBone ' A single bone of a mesh, a bone has a name by which it can be found in the frame hierarchy
	Field mName:TaiString ' The name of the bone
	Field mNumWeights:Int ' The number of vertices affected by this bone
	Field mWeights:TaiVertexWeight[] ' The vertices affected by this bone
	Field mOffsetMatrix:TaiMatrix4x4 ' Matrix that transforms from mesh space to bone space in bind pose
End Type

Type TaiAnimMesh ' NOT CURRENTLY IN USE. An AnimMesh is an attachment to an #aiMesh, stores per-vertex animations
	Field mVertices:TaiVector3[] ' Replacement for TaiMesh::mVertices
	Field mNormals:TaiVector3[] ' Replacement for TaiMesh::mNormals
	Field mTangents:TaiVector3[] ' Replacement for TaiMesh::mTangents
	Field mBitangents:TaiVector3[] ' Replacement for TaiMesh::mBitangents
	Field mColors:TaiColor4[AI_MAX_NUMBER_OF_COLOR_SETS] ' Replacement for TaiMesh::mColors
	Field mTextureCoords:TaiVector3[AI_MAX_NUMBER_OF_TEXTURECOORDS] ' Replacement for TaiMesh::mTextureCoords
    Field mNumVertices:Int ' The number of vertices in the TaiAnimMesh, and thus the length of all the member arrays
End Type

Type TaiMesh ' A mesh represents a geometry or model with a single material
	...
	Field mVertices:TaiVector3[] ' Vertex positions, always present in a mesh, mNumVertices in size
	Field mNormals:TaiVector3[] ' Vertex normals, normalized vectors, NULL if not present, mNumVertices in size
	Field mTangents:TaiVector3[] ' The tangent of a vertex points in the direction of the positive X texture axis
	Field mBitangents:TaiVector3[] ' The tangent of a vertex points in the direction of the positive Y texture axis
	Field mColors:TaiColor4[AI_MAX_NUMBER_OF_COLOR_SETS][] ' Vertex color sets, NULL if not present
	Field mTextureCoords:TaiVector3[AI_MAX_NUMBER_OF_TEXTURECOORDS][] ' Vertex texture coords, UV channels
	Field mNumUVComponents:Int[AI_MAX_NUMBER_OF_TEXTURECOORDS] ' Specifies the number of components for a given UV channel
	Field mFaces:TaiFace[] ' Each face refers to a number of vertices by their indices, mNumFaces in size
	Field mNumBones:Int ' The number of bones this mesh contains, if 0 the mBones array is NULL
	Field mBones:TaiBone[] ' A bone consists of a name by which it can be found in the frame hierarchy
	Field mMaterialIndex:Int ' The material used by this mesh, a mesh uses only a single material
	Field mName:TaiString ' Name of the mesh, meshes can be named but this is not a requirement
	Field mNumAnimMeshes:Int ' NOT CURRENTLY IN USE. The number of attachment meshes
	Field mAnimMeshes:TaiAnimMesh[] ' NOT CURRENTLY IN USE. Attachment meshes for this mesh, for vertex-based animation
End Type

Rem
bbdoc: Assimp mesh extended
about: A mesh represents a geometry or model with a single material.
...
*could* be missing if the AI_SCENE_FLAGS_INCOMPLETE flag is set in TaiScene::mFlags.
*/
EndRem
Type TaiMeshEx Extends TaiMesh
...
Type TaiNode ' Each node has a name, a parent node a transformation relative to its parent and possibly several child nodes
	Field mName:TaiString ' The name of the node
	Field mTransformation:TaiMatrix4x4 ' The transformation relative to the node's parent
	Field mParent:TaiNodeEx ' Parent node, Null if this node is the root node
	Field mNumChildren:Int ' The number of child nodes of this node
	Field mChildren:TaiNodeEx[] ' The child nodes of this node, Null if mNumChildren is 0
	...
	Field mMetaData:TaiMetadata ' Metadata associated with this node or Null if there is no metadata
End Type

Rem
bbdoc: Assimp node extended
EndRem
Type TaiNodeEx Extends TaiNode
	...
	Function Create:TaiNodeEx( pNode:Byte Ptr,parent:TaiNodeEx=Null )
	
		Local node:TaiNodeEx = New TaiNodeEx
		node.mName = New TaiString
		node.mParent = parent
		...
		node.mTransformation = TaiMatrix4x4Ex.Create(pOffset)
		...
		If node.mNumChildren > 0 ' get child nodes
			...
			For Local id:Int = 0 To node.mNumChildren - 1
				node.mChildren[id] = TaiNodeEx.Create(Byte Ptr( node.pChildren[id] ), node)
			Next
		EndIf
...
Type TaiScene ' The root structure of the imported data, everything that was imported from the file can be accessed from here
	Field mFlags:Int ' Any combination of the AI_SCENE_FLAGS_XXX flags, by default this value is 0
	Field mRootNode:TaiNodeEx ' The root node of the hierarchy
	Field mNumMeshes:Int ' The number of meshes in the scene
	Field mMeshes:TaiMeshEx[] ' The array of meshes
	Field mNumMaterials:Int ' The number of materials in the scene
	Field mMaterials:TaiMaterialEx[] ' The array of materials
	Field mNumAnimations:Int ' The number of animations in the scene
	Field mAnimations:TaiAnimation[] ' The array of animations
	Field mNumTextures:Int ' The number of textures embedded into the file
	Field mTextures:TaiTexture[] ' The array of embedded textures
	Field mNumLights:Int ' The number of light sources in the scene
	Field mLights:TaiLight[] ' The array of light sources
	Field mNumCameras:Int ' The number of cameras in the scene
	Field mCameras:TaiCamera[] ' The array of cameras
End Type

Rem
bbdoc: Assimp scene extended
EndRem
Type TaiSceneEx Extends TaiScene
	...
	Function LoadSceneFromMemory( scene:TaiSceneEx )
		...
		For Local id:Int = 0 To scene.mNumMeshes - 1
			...
			scene.mMeshes[id] = New TaiMeshEx
			...
			If scene.mMeshes[id].pVertices <> Null
				For Local n:Int = 0 To scene.mMeshes[id].mNumVertices - 1
					scene.mMeshes[id].mVertices[n] = New TaiVector3
			...
			If scene.mMeshes[id].pNormals <> Null
				For Local n:Int = 0 To scene.mMeshes[id].mNumVertices - 1
					scene.mMeshes[id].mNormals[n] = New TaiVector3
			...
			If scene.mMeshes[id].pTangents <> Null And scene.mMeshes[id].pBitangents <> Null
				For Local n:Int = 0 To scene.mMeshes[id].mNumVertices - 1
					scene.mMeshes[id].mTangents[n] = New TaiVector3
				...
				For Local n:Int = 0 To scene.mMeshes[id].mNumVertices - 1
					scene.mMeshes[id].mBitangents[n] = New TaiVector3
			...
			For Local n:Int = 0 To AI_MAX_NUMBER_OF_COLOR_SETS - 1
				...
				If pfloat <> Null And scene.mMeshes[id].pColors[n] <> Null
					For Local v:Int = 0 To scene.mMeshes[id].mNumVertices - 1
						scene.mMeshes[id].mColors[n][v] = New TaiColor4
			...
			For Local n:Int = 0 To AI_MAX_NUMBER_OF_TEXTURECOORDS - 1
				...
				If pfloat <> Null And scene.mMeshes[id].pTextureCoords[n] <> Null
					For Local v:Int = 0 To scene.mMeshes[id].mNumVertices - 1
						scene.mMeshes[id].mTextureCoords[n][v] = New TaiVector3
			...
			If scene.mMeshes[id].pFaces <> Null
				For Local n:Int = 0 To scene.mMeshes[id].mNumFaces - 1
					scene.mMeshes[id].mFaces[n] = New TaiFace
			...
			scene.mMeshes[id].mName = New TaiString
		...
		For Local id:Int = 0 To scene.mNumMaterials - 1
			scene.mMaterials[id] = New TaiMaterialEx
			...
			For Local pid:Int = 0 To scene.mMaterials[id].mNumProperties - 1
				scene.mMaterials[id].mProperties[pid] = TaiMaterialPropertyEx.Create(GetBytePtrAddress(pMaterialOffset))
				pMaterialOffset :+ SizeOf(scene.mMaterials[id].pProperties) ' 4/8
			Next

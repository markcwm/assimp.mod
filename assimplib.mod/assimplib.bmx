...
' post processing steps - enable a specific process by commenting it out, then edit source.bmx
...
' build settings - these are just for reference
...
' CMakeLists.txt - only OpenGEXImporter uses openddlparser
...
ModuleInfo "CC_OPTS: -DASSIMP_BUILD_NO_OWN_ZLIB" ' disable building internal zlib - uses older version than pub.zlib

' ImporterRegistry.cpp - enable a specific importer by commenting it out, then edit source.bmx
